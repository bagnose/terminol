...
cairo_font_face_t * X_FontSet::load(FcPattern * pattern, bool UNUSED(master)) throw (Error) {
    FcResult result;
    FcPattern * match = FcFontMatch(nullptr, pattern, &result);
    ENFORCE(match, "");

    // Note, the match will be free'd when the font is closed.
    cairo_font_face_t * font = cairo_ft_font_face_create_for_pattern(match);
    if (!font) {
...
void X_FontSet::unload(cairo_font_face_t * UNUSED(font)) {
    // No counterpart
}
