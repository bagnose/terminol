...
class Text final {
    ...
    public:
        ...
        static uint32_t maxSeqnum() { return std::numeric_limits<uint32_t>::max() >> 1; }
        ...
        void setContinued(bool continued) { _continued = static_cast<uint32_t>(continued); }
        ...
        void dump(std::ostream & ost, uint32_t indexSubtraction) const {
            ost << "index=" << _index - indexSubtraction << " seqnum=" << _seqnum << " "
...
public:
    ...
    Text(I_Repository & repository, int16_t rows, int16_t cols, uint32_t historyLimit);
    ...
    class I_Visitor {
    public:
        virtual void visitStyled(int32_t       row,
                                 int16_t       colBegin,
                                 int16_t       colEnd,
                                 const Style & style,
                                 const Para &  para,
        ...
        virtual void visitUnstyled(
            int32_t row, int16_t colBegin, int16_t colEnd, const Para & para, int32_t pos) = 0;
