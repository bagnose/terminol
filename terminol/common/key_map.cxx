...
namespace xkb {
    ...
    namespace {
        ...
        constexpr Map MAP_NORMAL[] = {{XKB_KEY_Left, 1, '[', 'D'},
                                      {XKB_KEY_Right, 1, '[', 'C'},
                                      {XKB_KEY_Up, 1, '[', 'A'},
                                      {XKB_KEY_Down, 1, '[', 'B'},
                                      {XKB_KEY_Home, 1, '[', 'H'},
        ...
        constexpr Map MAP_APPLICATION[] = {{XKB_KEY_Left, 1, 'O', 'D'},
                                           {XKB_KEY_Right, 1, 'O', 'C'},
                                           {XKB_KEY_Up, 1, 'O', 'A'},
                                           {XKB_KEY_Down, 1, 'O', 'B'},
                                           {XKB_KEY_Home, 1, 'O', 'H'},
        ...
        void normalise(xkb_keysym_t & keySym) {
            switch (keySym) {
            case XKB_KEY_KP_Space: keySym = XKB_KEY_space; break;
            case XKB_KEY_KP_Tab: keySym = XKB_KEY_Tab; break;
            ...
            case XKB_KEY_KP_Decimal: keySym = XKB_KEY_period; break;
            ...
            case XKB_KEY_KP_7:
            case XKB_KEY_KP_8:
            case XKB_KEY_KP_9: keySym = (keySym - XKB_KEY_KP_0) + XKB_KEY_0; break;
            default: break;
            }
        }

        void functionKeyResponse(
            char escape, int num, ModifierSet modifiers, char code, std::vector<uint8_t> & str) {
            ...
            if (modNum != 0) { ost << ESC << '[' << num << ';' << modNum + 1 << code; }
            else if (code != '~') {
                ost << ESC << escape << code;
            }
            else {
    ...
    std::string modifierToName(Modifier modifier) {
        ...
        case Modifier::SHIFT_LOCK: return "shift_lock";
        case Modifier::CAPS_LOCK: return "caps_lock";
        case Modifier::MODE_SWITCH: return "mode_switch";
        }
    ...
    bool isPotent(xkb_keysym_t keySym) {
        ...
        case XKB_KEY_Hyper_R: return false;
        default: return true;
        }
    }
    ...
    bool composeInput(xkb_keysym_t           keySym,
        ...
        case XKB_KEY_Escape: input.push_back(keySym & 0x7F); break;
        ...
        case XKB_KEY_Insert: functionKeyResponse('[', 2, modifiers, '~', input); break;

        case XKB_KEY_Delete:
            if (deleteSendsDel) { input.push_back(EOT); }
            else {
                functionKeyResponse('[', 3, modifiers, '~', input);
            }
            break;

        case XKB_KEY_Page_Up: functionKeyResponse('[', 5, modifiers, '~', input); break;

        case XKB_KEY_Page_Down: functionKeyResponse('[', 6, modifiers, '~', input); break;

        case XKB_KEY_F1: functionKeyResponse('O', 1, modifiers, 'P', input); break;

        case XKB_KEY_F2: functionKeyResponse('O', 1, modifiers, 'Q', input); break;

        case XKB_KEY_F3: functionKeyResponse('O', 1, modifiers, 'R', input); break;

        case XKB_KEY_F4: functionKeyResponse('O', 1, modifiers, 'S', input); break;

        case XKB_KEY_F5: functionKeyResponse('[', 15, modifiers, '~', input); break;

        case XKB_KEY_F6: functionKeyResponse('[', 17, modifiers, '~', input); break;

        case XKB_KEY_F7: functionKeyResponse('[', 18, modifiers, '~', input); break;

        case XKB_KEY_F8: functionKeyResponse('[', 19, modifiers, '~', input); break;

        case XKB_KEY_F9: functionKeyResponse('[', 20, modifiers, '~', input); break;

        case XKB_KEY_F10: functionKeyResponse('[', 21, modifiers, '~', input); break;

        case XKB_KEY_F12: functionKeyResponse('[', 24, modifiers, '~', input); break;

        default:
            auto convertUtf8 = true;

            // Handle special keys with alternate mappings.
            if (applyKeyMap(appCursor ? MAP_APPLICATION : MAP_NORMAL, keySym, modifiers, input)) {
                break;
            }

            if (modifiers.get(Modifier::CONTROL)) {
                if (keySym >= '3' && keySym <= '7') { keySym = (keySym & 0x1f) + 8; }

                if (!((keySym >= '!' && keySym <= '/') || (keySym >= '8' && keySym <= '?')
                      || (keySym >= '0' && keySym <= '2'))) {
                    keySym = keySym & 0x1f;
                }
                else if (keySym == '2') {
            ...
            if ((keySym < 0x80) || (!convertUtf8 && keySym < 0x100)) { input.push_back(keySym); }
            else {
                char buffer[16];
                auto ret = xkb_keysym_to_utf8(keySym, buffer, sizeof(buffer));

                if (ret == -1) { ERROR("Buffer to small to encode UTF-8 character."); }
                else if (ret == 0) {
