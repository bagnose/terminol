...
Terminal::Terminal(I_Observer &         observer,
    ...
    _priBuffer(_config,
               ...
               rows,
               cols,
               _config.unlimitedScrollBack ? std::numeric_limits<int32_t>::max()
...
bool Terminal::keyPress(xkb_keysym_t keySym, ModifierSet modifiers) {
    if (!handleKeyBinding(keySym, modifiers) && xkb::isPotent(keySym)) {
        ...
        if (xkb::composeInput(keySym,
            ...
            if (input.size() == 1 && _modes.get(Mode::META_8BIT) && modifiers.get(Modifier::ALT)) {
...
void Terminal::buttonPress(
    Button button, int count, ModifierSet modifiers, bool UNUSED(within), Pos pos, Hand hand) {
    ...
    if (_modes.get(Mode::MOUSE_PRESS_RELEASE)) {
        ...
        if (_modes.get(Mode::MOUSE_SELECT)) { goto select; }
        else {
            _press = Press::REPORT;
        }
    }
    ...
    select:
        if (button == Button::LEFT) {
            if (count == 1) { _buffer->markSelection(adjPos); }
            else {
    ...
    if ((_press == Press::REPORT && _modes.get(Mode::MOUSE_DRAG))
        ...
        if (within) {
            ...
            if (_modes.get(Mode::MOUSE_FORMAT_SGR)) {
                ost << ESC << "[<" << num << ';' << pos.col + 1 << ';' << pos.row + 1 << 'M';
            }
            else if (pos.row < 223 && pos.col < 223) {
                ost << ESC << "[M" << static_cast<char>(32 + num)
                    << static_cast<char>(32 + pos.col + 1) << static_cast<char>(32 + pos.row + 1);
            }
            else {
                // Couldn't deliver it
            }
            ...
            if (!str.empty()) { write(reinterpret_cast<const uint8_t *>(str.data()), str.size()); }
        }
    }
...
void Terminal::buttonRelease(bool UNUSED(broken), ModifierSet modifiers) {
    ...
    if (_press == Press::SELECT) {
        std::string text;
        if (_buffer->getSelectedText(text)) { _observer.terminalCopy(text, Selection::PRIMARY); }
...
void Terminal::scrollWheel(ScrollDir dir, ModifierSet modifiers, bool UNUSED(within), Pos pos) {
    if (_modes.get(Mode::MOUSE_PRESS_RELEASE)) {
        ...
        int16_t rows = modifiers.get(Modifier::SHIFT) ? 1 : std::max(1, _buffer->getRows() / 4);

        switch (dir) {
        case ScrollDir::UP:
            if (_buffer->scrollUpHistory(rows)) { fixDamage(Trigger::OTHER); }
            break;
        case ScrollDir::DOWN:
            if (_buffer->scrollDownHistory(rows)) { fixDamage(Trigger::OTHER); }
            break;
        }
    }
}
...
void Terminal::paste(const uint8_t * data, size_t size) {
    if (_config.scrollOnPaste && _buffer->scrollBottomHistory()) { fixDamage(Trigger::OTHER); }
...
void Terminal::focusChange(bool focused) {
    if (_focused != focused) {
        ...
        if (_modes.get(Mode::FOCUS)) {
            if (focused) { write(reinterpret_cast<const uint8_t *>("\x1B[I"), 3); }
            else {
                write(reinterpret_cast<const uint8_t *>("\x1B[O"), 3);
            }
        }

        if (_modes.get(Mode::SHOW_CURSOR)) { fixDamage(Trigger::FOCUS); }
    }
}
...
bool Terminal::handleKeyBinding(xkb_keysym_t keySym, ModifierSet modifiers) {
    ...
    if (iter != _config.bindings.end()) {
        ...
        case Action::WINDOW_TALLER:
            ...
            return true;
        case Action::LOCAL_FONT_RESET: _observer.terminalResizeLocalFont(0); return true;
        case Action::LOCAL_FONT_BIGGER: _observer.terminalResizeLocalFont(1); return true;
        case Action::LOCAL_FONT_SMALLER: _observer.terminalResizeLocalFont(-1); return true;
        case Action::GLOBAL_FONT_RESET: _observer.terminalResizeGlobalFont(0); return true;
        case Action::GLOBAL_FONT_BIGGER: _observer.terminalResizeGlobalFont(1); return true;
        case Action::GLOBAL_FONT_SMALLER: _observer.terminalResizeGlobalFont(-1); return true;
        ...
        case Action::SCROLL_UP_LINE:
            if (_buffer->scrollUpHistory(1)) { fixDamage(Trigger::OTHER); }
            return true;
        case Action::SCROLL_DOWN_LINE:
            if (_buffer->scrollDownHistory(1)) { fixDamage(Trigger::OTHER); }
            return true;
        case Action::SCROLL_UP_PAGE:
            if (_buffer->scrollUpHistory(_buffer->getRows())) { fixDamage(Trigger::OTHER); }
            return true;
        case Action::SCROLL_DOWN_PAGE:
            if (_buffer->scrollDownHistory(_buffer->getRows())) { fixDamage(Trigger::OTHER); }
            return true;
        case Action::SCROLL_TOP:
            if (_buffer->scrollTopHistory()) { fixDamage(Trigger::OTHER); }
            return true;
        case Action::SCROLL_BOTTOM:
            if (_buffer->scrollBottomHistory()) { fixDamage(Trigger::OTHER); }
            return true;
        ...
        case Action::DEBUG_HISTORY: _buffer->dumpHistory(std::cerr); return true;
        case Action::DEBUG_ACTIVE: _buffer->dumpActive(std::cerr); return true;
        ...
        case Action::DEBUG_SELECTION: _buffer->dumpSelection(std::cerr); return true;
        ...
        case Action::DEBUG_STATS2: {
            ...
            double dedupe = uniqueLines == 0 ? 0.0 : static_cast<double>(globalLines) / uniqueLines;

            std::ostringstream ost;
            ost << "local=" << localLines << " global=" << globalLines << " unique=" << uniqueLines
...
void Terminal::draw(Trigger trigger, Region & damage, bool & scrollbar) {
    ...
    if (trigger == Trigger::FOCUS) {
        ...
        if (trigger == Trigger::CLIENT) { _buffer->damageViewport(true); }
...
void Terminal::echo(const uint8_t * data, size_t size) {
    while (size != 0) {
        ...
        if (c == ESC) { processRead(reinterpret_cast<const uint8_t *>("^["), 2); }
        else if (c < SPACE) {
            if (c != LF && c != CR && c != HT) {
                c |= 0x40;
    ...
    if (size != 0) { processRead(data, size); }

    if (!_config.syncTty) { fixDamage(Trigger::TTY); }
}
...
void Terminal::sendMouseButton(int num, ModifierSet modifiers, Pos pos) {
    ...
    if (_modes.get(Mode::MOUSE_FORMAT_SGR)) {
        ...
        ost << ESC << "[M" << static_cast<char>(32 + num) << static_cast<char>(32 + pos.col + 1)
...
void Terminal::processRead(const uint8_t * data, size_t size) {
    for (size_t i = 0; i != size; ++i) {
        ...
        case utf8::Machine::State::ACCEPT:
            ...
            break;
        case utf8::Machine::State::REJECT: ERROR("Rejecting UTF-8 data."); break;
        default: break;
        }
    }
}
    ...
    for (size_t i = 0; i != args.size(); ++i) {
        ...
        case 38:
            ...
            if (i + 1 < args.size()) {
                i += 1;
                switch (args[i]) {
                case 0: NYI(<< "User defined foreground"); break;
                case 1: NYI(<< "Transparent foreground"); break;
                case 2:
                    if (i + 3 < args.size()) {
                ...
                case 5:
                    if (i + 1 < args.size()) {
                        i += 1;
                        auto v2 = args[i];
                        if (v2 >= 0 && v2 < 256) { _buffer->setFg(UColor::indexed(v2)); }
                        else {
        ...
        case 48:
            ...
            if (i + 1 < args.size()) {
                i += 1;
                switch (args[i]) {
                case 0: NYI(<< "User defined background"); break;
                case 1: NYI(<< "Transparent background"); break;
                case 2:
                    if (i + 3 < args.size()) {
                ...
                case 5:
                    if (i + 1 < args.size()) {
                        i += 1;
                        auto v2 = args[i];
                        if (v2 >= 0 && v2 < 256) { _buffer->setBg(UColor::indexed(v2)); }
                        else {
                            ERROR(<< "Colour out of range: " << v2);
                        }
                    }
            ...
            break;
        case 49: _buffer->setBg(UColor::stock(UColor::Name::TEXT_BG)); break;
    ...
    for (auto a : args) {
        if (priv == '?') {
            ...
            case 1003: // Mouse Any (Button press, drag, release, motion)
                ...
                if (set) { _modes.setTo(Mode::MOUSE_SELECT, false); }
                break;
            case 1004: _modes.setTo(Mode::FOCUS, set); break;
            ...
            case 2004: _modes.setTo(Mode::BRACKETED_PASTE, set); break;
            default:
            ...
            case 20: // LNM - Linefeed/new line
                ...
                break;
            default: WARNING("erresc: unknown set/reset mode: " << a); break;
            }
        }
        else {
            ERROR("?!");
...
const CharSub * Terminal::lookupCharSub(uint8_t code) {
    ...
    case 'K': // German
        NYI(<< "German");
        return nullptr;
    default: WARNING("Unknown character set: " << Char{code}); return nullptr;
    }
}
...
void Terminal::machineControl(uint8_t control) {
    switch (control) {
    case BEL: _observer.terminalBell(); break;
    case HT: _buffer->tabForward(1); break;
    case BS: _buffer->backspace(_modes.get(Mode::AUTO_WRAP)); break;
    case CR: _buffer->moveCursor2(true, 0, false, 0); break;
    case LF:
        if (_modes.get(Mode::CR_ON_LF)) { _buffer->moveCursor2(true, 0, false, 0); }
        [[fallthrough]];
    case FF:
    case VT: _buffer->forwardIndex(); break;
    case SO: _buffer->useCharSet(CharSet::G1); break;
    case SI: _buffer->useCharSet(CharSet::G0); break;
    default: break;
    }
}
    ...
    if (esc.inters.empty()) {
        ...
        case 'W': // CTC - Tabulator functions
            ...
            case 2:
                ...
                break;
            case 5: _buffer->clearTabs(); break;
        ...
        case 'g': // TBC
            switch (nthArg(esc.args, 0, 0)) {
            case 0: _buffer->unsetTab(); break;
            case 3: _buffer->clearTabs(); break;
            default: goto default_;
            }
            break;
        case 'h': // SM
        ...
        case 'm': // SGR - Select Graphic Rendition
            if (esc.args.empty()) { processAttributes({0}); }
            else {
        ...
        case 'r': // DECSTBM - Set Top and Bottom Margins (scrolling)
            if (esc.priv) { goto default_; }
            else {
                if (esc.args.empty()) { _buffer->resetMargins(); }
                else {
    ...
    if (!esc.args.empty()) {
        try {
            ...
            case 1: // Icon name
                ...
                break;
            case 2: // Window title
                if (esc.args.size() > 1) { _observer.terminalSetWindowTitle(esc.args[1], false); }
                break;
            case 55: NYI(<< "Log history to file"); break;
            case 112:
                // tmux gives us this...
                break;
...
void Terminal::bufferDrawBg(Pos pos, int16_t count, UColor color) {
    _observer.terminalDrawBg(pos, count, color);
}

void Terminal::bufferDrawFg(
...
void Terminal::bufferDrawCursor(
    Pos pos, UColor fg, UColor bg, AttrSet attrs, const uint8_t * str, size_t size, bool wrapNext) {
...
std::ostream & operator<<(std::ostream & ost, Terminal::Button button) {
    switch (button) {
    case Terminal::Button::LEFT: return ost << "left";
    case Terminal::Button::MIDDLE: return ost << "middle";
    case Terminal::Button::RIGHT: return ost << "right";
    }
...
std::ostream & operator<<(std::ostream & ost, Terminal::ScrollDir dir) {
    switch (dir) {
    case Terminal::ScrollDir::UP: return ost << "up";
    case Terminal::ScrollDir::DOWN: return ost << "down";
    }
