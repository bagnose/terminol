...
namespace {
    ...
    bool inRange(uint8_t c, uint8_t min, uint8_t max) { return c >= min && c <= max; }

} // namespace

VtStateMachine::VtStateMachine(I_Observer & observer, const Config & config)
    : _observer(observer), _config(config), _state(State::GROUND), _escSeq() {}
...
void VtStateMachine::consume(utf8::Seq seq, utf8::Length length) {
    if (length == utf8::Length::L1) {
        ...
        if (c == 0x18 /* CAN */ || c == 0x1A /* SUB */) {
            ...
            if (_state == State::OSC_STRING) { processOsc(_escSeq); }
    ...
    switch (_state) {
    case State::GROUND: ground(seq, length); break;
    case State::ESCAPE_INTERMEDIATE: escapeIntermediate(seq, length); break;
    case State::ESCAPE: escape(seq, length); break;
    case State::SOS_PM_APC_STRING: sosPmApcString(seq, length); break;
    case State::CSI_ENTRY: csiEntry(seq, length); break;
    case State::CSI_PARAM: csiParam(seq, length); break;
    case State::CSI_IGNORE: csiIgnore(seq, length); break;
    case State::CSI_INTERMEDIATE: csiIntermediate(seq, length); break;
    case State::OSC_STRING: oscString(seq, length); break;
    case State::DCS_ENTRY: dcsEntry(seq, length); break;
    case State::DCS_PARAM: dcsParam(seq, length); break;
    case State::DCS_IGNORE: dcsIgnore(seq, length); break;
    case State::DCS_INTERMEDIATE: dcsIntermediate(seq, length); break;
    case State::DCS_PASSTHROUGH: dcsPassthrough(seq, length); break;
    }
}
...
void VtStateMachine::ground(utf8::Seq seq, utf8::Length length) {
    if (length == utf8::Length::L1) {
        ...
        if (inRange(c, 0x00, 0x17) || c == 0x19 || inRange(c, 0x1C, 0x1F)) { processControl(c); }
        else if (inRange(c, 0x20 /* SPACE */, 0x7F /* DEL */)) {
            if (_config.traceTty) {
                std::cerr << CsiEsc::SGR(CsiEsc::StockSGR::FG_GREEN)
                          << CsiEsc::SGR(CsiEsc::StockSGR::UNDERLINE) << seq
...
void VtStateMachine::escape(utf8::Seq seq, utf8::Length length) {
    if (length == utf8::Length::L1) {
        ...
        if (inRange(c, 0x00, 0x17) || c == 0x19 || inRange(c, 0x1C, 0x1F)) { processControl(c); }
        else if (inRange(c, 0x30 /* 0 */, 0x4F /* O */) || inRange(c, 0x51 /* Q */, 0x57 /* W */)
                 || c == 0x59                      /* Y */
                 || c == 0x5A /* Z */ || c == 0x5C /* \ */
                 || inRange(c, 0x60 /* ` */, 0x7E /* ~ */)) {
            _escSeq.push_back(c);
...
void VtStateMachine::processControl(uint8_t c) {
    if (_config.traceTty) {
        std::cerr << CsiEsc::SGR(CsiEsc::StockSGR::FG_YELLOW) << Char{c}
                  << CsiEsc::SGR(CsiEsc::StockSGR::RESET_ALL);
        if (c == LF || c == FF || c == VT) { std::cerr << std::endl; }
    }
...
void VtStateMachine::processEsc(const std::vector<uint8_t> & seq) {
    ...
    if (_config.traceTty) {
        std::cerr << CsiEsc::SGR(CsiEsc::StockSGR::FG_CYAN) << esc.str()
                  << CsiEsc::SGR(CsiEsc::StockSGR::RESET_ALL);
    }
...
void VtStateMachine::processCsi(const std::vector<uint8_t> & seq) {
    ...
    if (_config.traceTty) {
        std::cerr << CsiEsc::SGR(CsiEsc::StockSGR::FG_WHITE) << esc.str()
                  << CsiEsc::SGR(CsiEsc::StockSGR::RESET_ALL);
    }
...
void VtStateMachine::processOsc(const std::vector<uint8_t> & seq) {
    ...
    for (auto c : seq) {
        if (next) {
            esc.args.push_back(std::string());
            next = false;
        }

        if (c == ';') { next = true; }
        else {
            esc.args.back().push_back(c);
        }
    }
    ...
    if (_config.traceTty) {
        std::cerr << CsiEsc::SGR(CsiEsc::StockSGR::FG_RED) << esc.str()
                  << CsiEsc::SGR(CsiEsc::StockSGR::RESET_ALL);
    }
