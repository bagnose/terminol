...
void Text::scrollDown(int16_t rowBegin, int16_t rowEnd, int16_t n) {
    ...
    if (rowBegin > 0) { makeUncontinued(rowBegin - 1); }

    if (rowEnd < getRows() - 1) { makeUncontinued(rowEnd - 1); }

    for (int16_t i = 0; i != n; ++i) {
        if (rowBegin < rowEnd - 1) { makeUncontinued(rowEnd - 2); }
...
void Text::makeContinued(int16_t row) {
    ...
    if (!_currentLines[row + _straddlingLines].isContinued()) {
        ...
        if (row < getRows() - 2 && _currentLines[row + 1 + _straddlingLines].isContinued()) {
...
void Text::makeUncontinued(int16_t row) {
    ...
    if (_currentLines[row + _straddlingLines].isContinued()) {
        ...
        for (uint32_t p0 = (thisLine.getSeqnum() + 1) * getCols(); p0 < thisPara.getLength();
             ++p0) {
...
void Text::cleanStraddling() {
    if (_straddlingLines > 0 && !_currentLines[_straddlingLines - 1].isContinued()) {
        auto tag = _repository.store(I_Repository::Entry{_currentParas.front().getStyles(),
                                                         _currentParas.front().getString()});
