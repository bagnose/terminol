...
protected:
    explicit Exception(const std::string & what_arg) : std::runtime_error(init(what_arg)) {}

private:
    static std::string init(std::string what_arg) {
        auto loc = get_thread_location();
        if (!loc.empty()) {
            if (what_arg.empty()) { what_arg = loc; }
            else {
                what_arg += ": " + loc;
            }
        }
...
public:
    explicit UserError(const std::string & message) : Exception(message), _message(message) {}
...
public:
    SystemError(std::error_code ec, const std::string & what_arg)
        : Exception(init(ec, what_arg)), _ec(ec) {}
...
#define THROW_IF_SYSCALL_FAILS(syscall, text)                 \
    (__extension__({                                          \
        ...
        if (rval_ == -1) { THROW_SYSTEM_ERROR(errno, text); } \
        rval_;                                                \
    }))
